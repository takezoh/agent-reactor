...
| **Pane** | The pty surface attached to a frame, served by `PtyBackend` over `platform/termvt`. Pane ids are synthetic (`%1`, `%2`, …) allocated by the termvt `Manager`; the browser xterm.js view subscribes to the same pane via the `server` gateway. |
...
| **Warm start** | Runtime startup against an existing `<dataDir>` — restores the frame stack from `sessions.json` and rebinds live panes; surviving containers are adopted. |
| **Cold start** | Runtime startup with no live panes (fresh boot / kill recovery) — respawns panes in root-to-tail order; surviving containers are discarded and provisioned fresh so `postCreate` daemons are guaranteed present. |
...
| Hook event target identification | Inject a frame-scoped env var at pane spawn time | Env vars are race-free at kernel exec level. See [state monitoring](state-monitoring.md#hook-event-routing-and-race-free-identification). |
...
- [Process model](process-model.md) — daemon process, pty pane model, rendering responsibilities
