...
For Codex, the daemon starts **one `codex app-server --listen unix://<runDir>/codex.sock` per container** (shared isolation: a single app-server services every project; project isolation: one per project; host mode: one per project, in its own cwd). The daemon connects to it via **WebSocket-over-UDS** (HTTP Upgrade handshake — the transport codex app-server speaks). Structured RPC events from the app-server are converted to `DEvSubsystem` and dispatched to the owning frame. Each frame's pane TUI connects with `codex --remote ws://127.0.0.1:8282` (cold start) or `codex resume <id> --remote ws://...` (warm start); `sockbridge` bridges the TCP loopback port to the UDS — in container mode it is started by the devcontainer `postCreate` (`runtime/subsystem/stream.ContainerBridgeSpec`), and in host mode by the stream backend itself. Codex state is not driven by hooks.
