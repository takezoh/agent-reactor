...
- `Command string` — a shell-joined string. Used by the **backend pane** launcher (the `server` backend, on top of `PtyBackend`).
...
| client (`server` backend) | `Command` (pane) | via `PtyBackend.SpawnWindow`; does not use `Spawn`, hands the command string to a pty-backed pane (ADR 0004) |
