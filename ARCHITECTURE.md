...
| **Subsystem** | Runtime-owned execution backend. `cli` launches a single command in a single pane; `stream` fronts long-lived structured backends such as Codex App Server. Subsystems own goroutines and I/O; Drivers are pure per-frame plugins with no I/O. Each `SessionFrame` carries both a `DriverState` (pure) and a `SubsystemID` (points to the runtime-owned backend) | Usually one tmux **pane** per subsystem |
...
runtime/subsystem/ Subsystem implementations — `cli` (passthrough) and `stream` (WebSocket-over-UDS backend for structured app-servers). Owns goroutines and I/O. The only location in `runtime/` permitted to import `driver/<tool>` (for tool-specific constants and socket paths)
