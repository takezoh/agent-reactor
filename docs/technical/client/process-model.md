...
For Codex, the daemon starts **one `codex app-server` per roost Session** (keyed by `stream:session:<sessionID>`). All frames within the same Session (root + peers) share one app-server; different Sessions get separate processes. The app-server is launched via `agentlaunch.Dispatcher.Wrap` + `agentlaunch.Spawn` (argv-direct, no host shell); the listen argv is built by `libcodex.AppServerListenArgs`. The host/container UDS path is chosen by `chooseSockPath()` — the stream daemon uses a dedicated non-TTY `DevcontainerLauncher` (`docker exec -i`) that shares the same `sandbox.Manager` as the TUI pane launcher. The daemon connects via **WebSocket-over-UDS** (HTTP Upgrade — the transport codex app-server speaks). Structured RPC events are converted to `DEvSubsystem` and dispatched to the owning frame. Each frame's pane TUI connects with `codex --remote ws://127.0.0.1:8282/<sessionID>` (cold start) or `codex resume <id> --remote ws://...` (warm start); these remote-attach strings are built by `libcodex.RemoteAttachArgs`. A routing bridge maps the TCP loopback port to per-session UDS files (`codex-<sessionID>.sock`): in container mode it is started by the devcontainer `postCreate` hook as `roost-bridge sockbridge -route-dir <runDir>` (already bind-mounted as part of the run dir); in host mode the daemon starts it as a singleton goroutine at startup. Codex state is not driven by hooks. When a session's last frame is released, `Factory.Remove` stops the corresponding app-server process (reap on exit).
