...
runtime/subsystem/ Subsystem interface and shared worktree utilities (`subsystem.CreateWorktree`, `RemoveWorktree`, `CleanupUntracked`, `GenerateWorktreeNames`, `IsManagedWorktreePath`). Subsystem implementations — `cli` (per-frame process and worktree lifecycle) and `stream` (WebSocket-over-UDS backend for structured app-servers). Owns goroutines and I/O. The only location in `runtime/` permitted to import `driver/<tool>` (for tool-specific constants and socket paths)
...
| Resident tracking | `SubsystemID -> PaneID`, `FrameID -> TargetID`, `FrameID -> Subsystem` | Visible panes belong to subsystems; frames track logical execution targets. The `frameSubsystems` sync.Map routes `ReleaseFrame` calls from `executeKillSessionWindow` to the owning subsystem without needing to know the subsystem kind. CLI uses one frame per subsystem, while Codex maps many frames (threads) onto one stream subsystem pane |
...
| Structured stream integration | `codex app-server` → bridge/gateway → `proto.CmdSubsystemEvent` → `EvSubsystem` → `reduceSubsystem` → `Driver.Step(DEvSubsystem)` | Used by Codex. The daemon starts one `codex app-server --listen unix://<runDir>/codex.sock` per project and connects via **WebSocket-over-UDS** (`runtime/subsystem/stream/`). `sockbridge` bridges TCP loopback port 8282 to the UDS for the TUI's `--remote ws://` URL. `LaunchSubsystem` (`cli` / `stream`) distinguishes CLI-spawned agents from stream-backed ones; the `stream` subsystem implementation handles the codex-specific binding. The stream layer emits structured tool / approval / plan / diff / message / thread lifecycle events, and `TargetID` carries the logical thread identity |
